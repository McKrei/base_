...
from . import app, db
from .forms import NewsForm, EnterEmail, SearchForm
...
def history_search():
    form = EnterEmail()
    email = None
    news_list = Query.query.all()
    if form.validate_on_submit():
        email = form.email.data
        news_list = news_list.filter_by(email=email)
        print(email)
    return render_template('history_search.html', form=form, email=email, news_list=news_list)
